...
class AnswerVotes extends React.Component {
    ...
    votingDisplay() {
        return (
            <div className="a-vote">
                <TiArrowSortedUp
                    id="up-a-vote"
                    className={`up-${this.props.answer.id}`}
                    onClick={this.handleUpvote}
                />
                <p className="voting-score">{this.state.votes}</p>
                <TiArrowSortedDown
                    id="down-a-vote"
                    className={`down-${this.props.answer.id}`}
    ...
    indicateVote() {
        if (this.props.currentUser) {
            ...
            let upvoteArrow = document.getElementsByClassName(`up-${this.props.answer.id}`)[0];
            let downvoteArrow = document.getElementsByClassName(`down-${this.props.answer.id}`)[0];
